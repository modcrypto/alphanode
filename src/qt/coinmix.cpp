...
class TxViewDelegate2 : public QAbstractItemDelegate
{
    Q_OBJECT
public:
    TxViewDelegate2() : QAbstractItemDelegate(), unit(BitcoinUnits::ALN)
    {
    }
...
void Coinmix::updateDarksendProgress()
{
    ...
    QString strAnonymizeALNAmount = BitcoinUnits::formatHtmlWithUnit(nDisplayUnit, nAnonymizeALNAmount * COIN, false, BitcoinUnits::separatorAlways);
    ...
    if (currentBalance == 0) {
        ...
        strAnonymizeALNAmount = strAnonymizeALNAmount.remove(strAnonymizeALNAmount.indexOf("."), BitcoinUnits::decimals(nDisplayUnit) + 1);
        strAmountAndRounds = strAnonymizeALNAmount + " / " + tr("%n Rounds", "", nDarksendRounds);
    ...
    if (nMaxToAnonymize > nAnonymizeALNAmount * COIN) nMaxToAnonymize = nAnonymizeALNAmount * COIN;

    if (nMaxToAnonymize == 0) return;

    if (nMaxToAnonymize >= nAnonymizeALNAmount * COIN) {
        ui->labelAmountRounds->setToolTip(tr("Found enough compatible inputs to anonymize %1")
                                              .arg(strAnonymizeALNAmount));
        strAnonymizeALNAmount = strAnonymizeALNAmount.remove(strAnonymizeALNAmount.indexOf("."), BitcoinUnits::decimals(nDisplayUnit) + 1);
        strAmountAndRounds = strAnonymizeALNAmount + " / " + tr("%n Rounds", "", nDarksendRounds);
    } else {
